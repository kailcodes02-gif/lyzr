...
- `public/_headers`:
  ...
  ```
  No Cross-Origin-Opener-Policy anywhere. The bridge page must be iframe-able by its own origin for `ssoSilent`. Tune hosts after observing real traffic (M5). Correction 2026-09-20: `script-src 'self'` alone breaks a static export (Next emits inline hydration scripts); the staging script hashes them instead. The `_headers` file must sit at the assets root (`dist/_headers`), not under `dist/MS/`.
...
| XSS steals 24 h tokens from localStorage | Significant | Zero third-party scripts; CSP with sha256 hashes for Next's inline scripts (no 'unsafe-inline' for script-src, generated by scripts/stage.mjs); DOMPurify + script-less sandboxed iframe for email HTML. Residual: any XSS in this origin still yields the cache, so keep dependencies minimal and re-check the hash list after every Next upgrade |
