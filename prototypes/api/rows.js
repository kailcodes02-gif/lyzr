// =============================================================================
// Vercel Serverless Function: POST /api/rows + PUT /api/rows?id=xxx
// Handles adding new rows and editing existing rows in the shared data.json file.
// Implements Google OAuth2 verification & optimistic concurrency conflict retries.
// =============================================================================
  ...
  while (attempt < maxRetries) {
    ...
    try {
      ...
      if (req.method === 'POST') {
        ...
        await ghPut(owner, repo, token, ghPath, data, sha, `data: add "${newRow.project}" [${newRow.id}] by ${user.email} (prototypes)`, branch);
        ...
        await ghPut(owner, repo, token, ghPath, data, sha, `data: edit "${existing.project}" [${rowId}] by ${user.email} (prototypes)`, branch);
        return res.status(200).json({ success: true, row: existing, changes });
      }

    } catch (err) {
      console.warn(`Attempt ${attempt} failed:`, err.message);
      const isConflict = err.message.includes('409') || err.message.includes('422') || err.message.toLowerCase().includes('conflict');
