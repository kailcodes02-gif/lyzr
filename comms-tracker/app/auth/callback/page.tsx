...
import { DRIVE_SCOPE, GMAIL_SCOPE } from "@/lib/hooks/use-drive-connect";

// PKCE landing page (static build — no server route): exchanges the ?code
// from Google/Supabase for a session, then enters the app.
//
// `?connect=google` (older links: `drive`) marks this as a Drive+Gmail
// incremental-authorization round trip (see the "Connect Google" button on
// /admin/sync) rather than a fresh sign-in — the session's provider_refresh_token is ONLY ever
...
function CallbackContent() {
  ...
  useEffect(() => {
    ...
    const finish = async () => {
      ...
      if (connecting === "google" || connecting === "drive") {
        if (!session.provider_refresh_token) {
          toast.error("Google didn't return a refresh token — try connecting again");
        } else {
          try {
            const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_PATH || ""}/api/oauth/google/connect`, {
              method: "POST",
            ...
            toast.success("Google Drive + Gmail connected");
          } catch (err) {
            toast.error(`Failed to save Google connection: ${err instanceof Error ? err.message : String(err)}`);
          }
        }
