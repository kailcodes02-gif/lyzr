...
export default function DashboardPage() {
  ...
  // The space dashboard needs a vertical; the workspace home lives at "/".
  useEffect(() => {
  ...
  // Vertical budget first, workspace-global as fallback
  const globalBudget = budgets?.find(b => b.scope_type === 'vertical' && b.scope_id === vertical?.id) || budgets?.find(b => b.scope_type === 'global')
    ...
    <div className="p-4 lg:p-8 space-y-8 max-w-7xl mx-auto bg-zinc-50 text-zinc-900 min-h-screen">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
          <Link href={href('/calendar/')} className="flex-1 sm:flex-none">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card className="bg-white border-zinc-200 backdrop-blur-xl hover:border-zinc-300 transition-all">
          <CardContent className="p-6 flex items-center justify-between">
            <div>
              <p className="text-xs font-medium text-zinc-500 uppercase tracking-wider">{globalBudget?.scope_type === 'vertical' ? 'Vertical Budget Left' : 'Global Budget Left'}</p>
