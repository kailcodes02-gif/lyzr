...
export default function BudgetsPage() {
  ...
  if (!activeLabel) {
    return (
      <div className="p-8 text-center text-zinc-500 bg-[#0a0a0f] min-h-screen">
    ...
    <div className="p-4 lg:p-8 space-y-8 max-w-7xl mx-auto bg-[#0a0a0f] text-zinc-100 min-h-screen">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-white flex items-center gap-2">
            <Landmark className="w-6 h-6 text-emerald-400" /> Budgets
          </h1>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <SelectTrigger className="w-[180px] bg-zinc-900 border-white/10 text-xs">
              <SelectValue placeholder="Select Period" />
            </SelectTrigger>
            <SelectContent className="bg-zinc-900 border-white/10 text-xs text-zinc-300">
        ...
        <Card className="bg-zinc-900/40 border-white/5 backdrop-blur-xl">
          <CardHeader className="pb-3 border-b border-white/5">
            <div className="flex items-center justify-between">
              <CardTitle className="text-base font-semibold text-white flex items-center gap-2">
                <DollarSign className="w-4 h-4 text-emerald-400" /> Global Budget Summary
              </CardTitle>
          ...
          <CardContent className="p-6 grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="space-y-1">
              ...
              <h2 className="text-3xl font-bold text-white">${totalGlobalBudget.toLocaleString()}</h2>
            </div>
            <div className="space-y-1">
              <span className="text-[11px] text-zinc-500 uppercase tracking-wider font-semibold">Total Allocated</span>
              <h2 className="text-3xl font-bold text-emerald-400">${allocatedGlobalBudget.toLocaleString()}</h2>
            </div>
            <div className="space-y-1">
              <span className="text-[11px] text-zinc-500 uppercase tracking-wider font-semibold">Remaining Funds</span>
              <h2 className={`text-3xl font-bold ${remainingGlobalBudget < 0 ? 'text-red-400' : 'text-zinc-400'}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="bg-zinc-900/40 border-white/5 backdrop-blur-xl">
          <CardHeader>
            <CardTitle className="text-sm font-bold uppercase tracking-wider text-zinc-400 flex items-center gap-2">
              <Layers className="w-4 h-4 text-blue-400" /> Category Scopes
            </CardTitle>
          </CardHeader>
                ...
                return (
                  <div key={cb.budget_period_id} className="space-y-2">
                    <div className="flex justify-between items-center text-xs">
                      <span className="font-semibold text-zinc-300">{catName}</span>
                      <span className="text-zinc-400">
                        ${alloc.toLocaleString()} / ${cap.toLocaleString()} ({pct}%)
                      </span>
                    </div>
                    <Progress value={pct} className="h-1.5 bg-zinc-800" indicatorClassName="bg-blue-500" />
                  </div>
                )
              })
            )}
        ...
        <Card className="bg-zinc-900/40 border-white/5 backdrop-blur-xl">
          <CardHeader>
            <CardTitle className="text-sm font-bold uppercase tracking-wider text-zinc-400 flex items-center gap-2">
              <Tag className="w-4 h-4 text-violet-400" /> Channel Scopes
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
              ...
              channelBudgets.map(chb => {
                ...
                return (
                  <div key={chb.budget_period_id} className="space-y-2">
                    <div className="flex justify-between items-center text-xs">
                      <span className="font-semibold text-zinc-300">{chName}</span>
                      <span className="text-zinc-400">
                        ${alloc.toLocaleString()} / ${cap.toLocaleString()} ({pct}%)
                      </span>
                    </div>
                    <Progress value={pct} className="h-1.5 bg-zinc-800" indicatorClassName="bg-violet-500" />
                  </div>
                )
              })
            )}
      ...
      <Card className="bg-zinc-900/40 border-white/5 backdrop-blur-xl">
        <CardHeader>
          <CardTitle className="text-base font-semibold text-white">Budgeted Tasks Allocation List</CardTitle>
        </CardHeader>
            ...
            <tbody className="divide-y divide-white/5">
              {budgetedTasks.map(task => (
                ...
                >
                  ...
                  <td className="py-3.5 px-4 text-zinc-400">{task.channel?.name || '-'}</td>
